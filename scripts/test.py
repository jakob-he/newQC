...
class JsonFile:
    ...
    old_schema = {
        'submitter': {
            'team': '', 'name': ''
        }, 'vcf': '', 'geneList': [{'gestalt_score': ''}], 'features': '', 'ranks': '', 'genomicData': [
            {'Mutations': {'HGVS-code': '', 'Mutation 1': ''}, 'Test Information': {'Mutation Type': ''}
             }
        ]
    }
    ...
    def check(self, false_only=True):
        schema = self.check_schema(self.old_schema, self.raw)
        self.error = schema
        if false_only:
...
test = JsonFile(
    '/home/jakobh/Dokumente/PEDIA/PEDIA-workflow-new_download/1_qualityCheck/json/currated/885.json')
test.check()
