...
if __name__ == '__main__':
    ...
    parser.add_argument('-j', '--jsonsoriginal',
                        help='path of original json folder')
    parser.add_argument('-m', '--mappedjsons',
                        help='path of mapped json folder')
    ...
    for fileName in os.listdir(case_path):
        ...
        if test not in os.listdir(newpath) and os.path.isfile(case_path + test):
            ...
            file_content['vcf'] = rename_document_vcf(
                file_content['documents'], file_content['case_id'])
            if not file_content['vcf']:
                file_content["vcf"] = "noVCF"
            for syndrome in file_content["detected_syndromes"]:
                ...
                try:  # debugging
                    ...
                    with open("Mappingexceptions.txt", "a") as exceptions:
                        exceptions.write(
                            str(fileName) + "\t" + syndromename + "\n")
        else:
            print("This file has already been mapped")
